...
const UploadPaper = () => {
  ...
  const ec = new EC("p256"); // Initialize elliptic curve
  ...
  async function encryptDecryptAndDownload() {
    try {
      ...
      // console.log("Ciphertext received:", JSON.stringify(ciphertext, null, 2));
      const encryptedBlob = new Blob([JSON.stringify(encryptedData, null, 2)], {
      ...
      // Dummy values from your example
      const participantShares = {
        1: new BN(
          "37706708683060499824552293360004594715189574732727554988563076382707584221917"
        ),
        2: new BN(
          "31539101773781103867561353008408501730551045966246325417326002635753400207091"
        ),
        3: new BN(
          "25371494864501707910570412656812408745912517199765095846088928888799216192265"
        ),
        4: new BN(
          "19203887955222311953579472305216315761273988433283866274851855141845032177439"
        ),
        5: new BN(
          "13036281045942915996588531953620222776635459666802636703614781394890848162613"
        ),
      };

      console.log("polynomial generation function being called...");
      generatePolynomialAndCommitments(5);
      console.log("polynomial generation done....");
      console.log("asymmetric encryption starts.....");
      const keyPair = ec.genKeyPair();
      const pubKey = keyPair.getPublic("hex");

      console.log("reconstruction of key starts....");

      const reconkey = reconstructKey(participantShares, 3);
      console.log("reconstruction done....");

      const result = computeOverallPublicKey(participantShares);
      console.log("Overall Public Key:", result);
      const encryptedDataasym = asymmetricEncryption(result, sessionKey);
      console.log("C2 length:", encryptedDataasym.C2.length); // Should be 64 hex chars (32 bytes)
      if (encryptedDataasym.C2.length < 64) {
        encryptedDataasym.C2 = encryptedDataasym.C2.padStart(64, "0"); // Pad with leading zeros
      }
      console.log("asymmetric encryption done.....");

      console.log("asymmetric decryption starts.....");
      const decryptedMessage = asymmetricDecrypt(reconkey, encryptedDataasym);
      console.log("Decrypted message:", decryptedMessage.toString(16));
      console.assert(
        decryptedMessage === sessionKey,
        `Decryption failed!\nExpected: ${sessionKey}\nGot: ${decryptedMessage}`
      );

      const participants = setupParticipants(5, 2);
      console.log("Participant 0's polynomial:", participants[0].coefficients);
      console.log("Share for P2 from P0:", participants[0].shares[2]);
      const isValid = verifyShare(
        participants[0].shares[2],
        2,
        participants[0].commitments
      );
      console.log("Is share valid?", isValid);
  ...
  async function signAndVerify(hash) {
    ...
    // 3. Verify Signature
    const isValid = keyPair.verify(hashedCiphertext, derSignature);
